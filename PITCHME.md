...
- @size[xxx-large](**scala library newtype**)
  - @size[xx-large](Haskell language provides a newtype keyword for creating new types from existing ones without runtime overhead.)
  - @size[xx-large](The newtype scala library gives us zero-cost wrappers with no runtime overhead relying on macros.)
    - @size[x-large](libraryDependencies += "io.estatico" %% "newtype" % "0.4.3")
    - @size[x-large](addCompilerPlugin("org.scalamacros" % "paradise" % "2.1.1" cross CrossVersion.full))
  - @size[xx-large](Newtypes will eventually be replaced by opaque types (Dotty, a subset in 2.14))
@ulend
@snapend
